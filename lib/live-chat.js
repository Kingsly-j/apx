...
export const SUPPORT_EMAIL = 'support@bluecrestlogistics.com';
...
export function buildAnswerForQuestion(question = '') {
  ...
  if (/(contact|call|email|support|phone|whatsapp)/.test(normalized)) {
    return 'You can contact Bluecrest Logistics by phone at +1 (915) 201-9157, WhatsApp, or email at support@bluecrestlogistics.com.';
  }

  if (/(payment|pay|paid|storage fee|arrange payment)/.test(normalized)) {
    return 'You can arrange payment by sending the fee details through WhatsApp or by emailing support@bluecrestlogistics.com with your tracking number and payment request.';
  }
