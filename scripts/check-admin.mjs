...
try {
  ...
  await expect(page.getByText(/Firebase (did not confirm|is unavailable)/)).toBeVisible({timeout:20000});
  ...
  await expect(page.getByText('Admin added: test-operator@example.com')).toBeVisible();
  const form = page.locator('form').filter({has:page.getByRole('heading',{name:'New cargo file'})});
  await form.getByLabel('Customer name',{exact:true}).fill('Verification customer');
  ...
  await expect(page.getByText(/new row violates row-level security policy/)).toBeVisible();
  ...
  await expect(page.getByText(/Firebase (did not confirm|is unavailable)/)).toBeVisible({timeout:20000});
  await expect(page.getByRole('button',{name:'Copy code'})).toHaveCount(0);
  await expect(form.getByLabel('Customer name',{exact:true})).toHaveValue('Verification customer');
