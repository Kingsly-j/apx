...
export default function ShipmentRoute({shipment}:{shipment:Shipment}) {
  ...
  if(!stops.length) {
    ...
    return localize(<div><iframe title="Complete shipment route map" src={src} loading="lazy" className="h-[420px] w-full border-0" referrerPolicy="strict-origin-when-cross-origin"/><div className="route-caption"><span>{locations.join(" \u2192 ")}</span><a href={routeLink} target="_blank" rel="noopener noreferrer">Open route in maps</a></div></div>);
  }
