...
export default function ShipmentRoute({shipment}:{shipment:Shipment}) {
  ...
  const stops=(shipment.routeStops ?? []).filter(stop=>Number.isFinite(stop.latitude)&&Number.isFinite(stop.longitude)&&Math.abs(stop.latitude)<=85&&Math.abs(stop.longitude)<=180);
  const routeLink=`https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(shipment.origin)}&destination=${encodeURIComponent(shipment.destination)}&waypoints=${encodeURIComponent(stops.filter(stop=>stop.kind!=="origin"&&stop.kind!=="destination").map(stop=>`${stop.latitude},${stop.longitude}`).join("|"))}`;
  if(!stops.length) {
    const locations=[shipment.origin,shipment.location,shipment.destination].filter(Boolean);
    const src=`https://maps.google.com/maps?output=embed&saddr=${encodeURIComponent(locations[0] ?? "")}&daddr=${encodeURIComponent(locations.slice(1).join(" to:"))}`;
    return <div><iframe title="Complete shipment route map" src={src} loading="lazy" className="h-[420px] w-full border-0" referrerPolicy="strict-origin-when-cross-origin"/><div className="route-caption"><span>{locations.join(" \u2192 ")}</span><a href={routeLink} target="_blank" rel="noopener noreferrer">Open route in maps</a></div></div>;
  }
  ...
  return <div><svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`Shipment route: ${stops.map(stop=>stop.label).join(' to ')}`} className="block w-full bg-[#e9f0f4]"><title>Complete shipment route</title>{tiles}<polyline points={normalized.map(p=>`${p.x*scale-left},${p.y*scale-top}`).join(' ')} fill="none" stroke="#facc15" strokeWidth="5" strokeLinejoin="round"/>{normalized.map((stop,index)=><g key={stop.id}><circle cx={stop.x*scale-left} cy={stop.y*scale-top} r="12" fill={colors[stop.kind]} stroke="white" strokeWidth="3"/><text x={stop.x*scale-left} y={stop.y*scale-top+4} textAnchor="middle" fill="white" fontSize="11" fontWeight="bold">{index+1}</text><title>{stop.label} ({stop.kind})</title></g>)}</svg><div className="route-caption"><span>© <a href="https://www.openstreetmap.org/copyright" target="_blank" rel="noopener noreferrer">OpenStreetMap</a> contributors</span><a href={routeLink} target="_blank" rel="noopener noreferrer">Open route in maps</a></div>{failed&&<p className="px-5 text-sm text-slate-600">Map tiles are unavailable. Route markers and the stop list remain visible.</p>}<ol className="route-stop-list">{stops.map((stop,index)=><li key={stop.id}><span style={{background:colors[stop.kind]}}>{index+1}</span><div><strong>{stop.label}</strong><small>{stop.kind === 'current' ? 'Current location' : stop.kind}</small></div></li>)}</ol><p className="px-5 pb-3 text-sm text-slate-600">Estimated distance: {shipment.estimatedDistance || "Not provided"}</p><p className="px-5 pb-4 text-xs text-slate-500">Route markers show recorded locations, not continuous GPS tracking.</p></div>;
}
