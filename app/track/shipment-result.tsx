...
function Icon({name}:{name:string}) {return <i className={`fas fa-${name}`} aria-hidden="true"/>;}
export function displayDate(value?:string,withTime=false) {
  if(!value)return "Not recorded";
  const date=new Date(value);
  if(!Number.isFinite(date.getTime()))return value;
  return new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',...(withTime?{hour:'2-digit',minute:'2-digit'} as const:{})}).format(date);
}
function Badge({status}:{status:ShipmentStatus}) {const label=statusLabel(status);return <span className={`tracking-badge ${label==='Delivered'?'delivered':label==='Custom Hold'?'hold':''}`}>{label}</span>;}
function Row({icon,label,value}:{icon:string;label?:string;value?:ReactNode}) {return <div className="tracking-info-row"><Icon name={icon}/><div>{label&&<span className="tracking-label">{label}: </span>}{value || <span className="text-slate-400">Not provided</span>}</div></div>;}
function Card({title,icon,children}:{title:string;icon:string;children:ReactNode}) {return <section className="tracking-info-card"><h3><Icon name={icon}/>{title}</h3><div className="grid gap-3">{children}</div></section>;}
...
export default function ShipmentResult({shipment}:{shipment:Shipment}) {
  ...
  return <div className="tracking-result" aria-label="Shipment details">
    ...
    <section className="tracking-surface"><h2 className="tracking-section-title">Shipment Progress</h2><ol className="tracking-milestones" aria-label="Shipment progress">{trackingMilestones.map((status,index)=>{
      const date=shipment.milestoneDates?.[status] ?? history.find(event=>statusLabel(event.status)===status)?.date;
      const reached=index<=currentIndex;
      return <li key={status} className={`${reached?'reached':''} ${status==='Custom Hold'?'hold':status==='Delivered'?'delivered':''}`} aria-current={status===current?'step':undefined}><span className="milestone-icon"><Icon name={['check-circle','box','truck','pause-circle','check-double'][index]}/></span><strong>{status}</strong><time>{displayDate(date)}</time></li>;
